...
class BioProcess(InProcessMixin,
    ...
    def __init__(self, 
        ...
        df = md.exp_info
        self._cell_line = df['cell_line'].to_string(index=False)
        self._exp_id = df['exp_id'].to_string(index=False)
        self._experimenter_name = df['name'].to_string(index=False)
        self._initial_volume = df['initial_volume'].to_string(index=False)
        self._unit = df['unit'].to_string(index=False)
        ...
        species_dict = metabolite(spc_list=self._spc_list, measured_data=self._md)
        species_dict["CELL"] = Cell(name='cell', measured_data=md)
        species_dict["OXYGEN"] = Oxygen(name='oxygen', measured_data=md)
        species_dict["PRODUCT"] = Product(name='product', measured_data=md)
        self._spc_dict = species_dict
        
        # List for Nitrogen, AA carbon
        self._spc_list_2 = []
        
        # Bioprocess data df
        self._process_data_dict = {'prepro'  : md.get_pre_data(),
                                   'inpro'   : pd.DataFrame(),
                                   'twopt'   : pd.DataFrame(),
...
#####################################################################################################################
# Default Species list
default_spc_name_dict = {'Alanine'   : 'ALA', 'Arginine'      : 'ARG', 'Asparagine'   : 'ASN',
...
default_spc_list = list(default_spc_name_dict.keys())
