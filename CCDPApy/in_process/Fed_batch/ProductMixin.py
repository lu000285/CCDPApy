...
class ProductMixin(GetterMixin):
    ...
    def cumulative_calc(self) -> pd.DataFrame:
        ...
        v1 = self.volume_before_sampling['value'].values[idx]# Culture Volume Bfore sampling (ml)
        v2 = self.volume_after_sampling['value'].values[idx]# Culture Volume After feeding (ml)
