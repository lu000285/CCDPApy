...
class MeasuredData:
    ...
    def __init__(self, experiment_info, raw_data):
        ...
        #
        self._sample_volume = check_key(raw_data, 'SAMPLE VOLUME (mL)')
        self._feed_media_added = check_key(raw_data, 'FEED MEDIA ADDED (mL)')
        #
        # self._glucose_feed_added = check_key(raw_data, 'GLUCOSE ADDED (mL)')
        self._glutamine_feed_added = check_key(raw_data, 'GLUTAMINE FEED ADDED (mL)')
        #
        self._base_added = check_key(raw_data, 'BASE ADDED (mL)')
        self._xv = check_key(raw_data, 'VIABLE CELL CONC. XV (x106 cells/mL)')  # xv: Viable Cell Concentration
        self._xd = check_key(raw_data, 'DEAD CELL CONC. Xd (x106 cells/mL)')    # xd: Dead Cell Concentraion
        self._xt = check_key(raw_data, 'TOTAL CELL CONC. Xt (x106 cells/mL)')   # xt: Total Cell Concentraion
        self._viability = check_key(raw_data, 'VIABILITY (%)')
        self._pH = check_key(raw_data, 'pH')
        self._do = check_key(raw_data, 'DO (%)')
        self._our = check_key(raw_data, 'OUR (mmol/L/hr)')
        self._oxygen_consumption_rate = check_key(raw_data, 'SP. OXYGEN CONSUMPTION RATE (mmol/109cell/hr)')
        self._oxygen_consumed = check_key(raw_data, 'OXYGEN CONSUMED (mmol/L)')
        ...
        # self._glucose_feed_added = pd.Series(data=[0.0]*n) #######
        ...
        # self.culture_volume()
        a = [1935.00, 1920.00, 1905.00, 1876.00, 1861.00, 1872.48, 1883.78, 1895.81, 1920.96, 1918.81, 1862.81, 1853.06, 1808.22, 1802.39, 1800.22]
        self._v_before_sampling = pd.Series(data=a)
        self._v_after_sampling = pd.Series(data=a)
    ...
    def culture_volume(self):
        ...
        for i in range(n):
            ...
            supplements_added = self._base_added.iat[i] + self._glucose_feed_added.iat[i] + self._feed_media_added.iat[i]
